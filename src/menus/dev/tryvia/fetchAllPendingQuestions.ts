...
export function fetchAllPendingQuestionsMenu<R>(questions: TryviaQuestions[], page: number = 0): R {
    ...
    const rows = [
        ...
        createRow(
            new StringSelectMenuBuilder({
                customId: "devMenu/tryvia/approveMany",
                placeholder: "Aprovar Perguntas",
                options: pageQuestions.map(q => ({
                    label: q.question.slice(0, 100),
                    description: `ID: ${q.id} | Status: ${q.status} | Dificuldade: ${q.difficulty}`,
                    value: q.id.toString()
