...
export default async function takeStx(app: FastifyInstance, client: Client<true>) {
    app.post("/take-stx", async (req, reply) => {
        if (!req.application || ( !req.application.data.permissions.includes("ALL") && !req.application.data.permissions.includes("ECONOMY.WRITE") )) {
            return reply.status(StatusCodes.FORBIDDEN).send({ message: "You do not have permission to take stx to a user", success: false });
        }
        ...
        try {
            ...
            const [_, transaction] = await Promise.all([
                redis.setex(`tx:${memberId}:${application.data.id}`, Math.min(convertTime({ time: expiresAt, to: "seconds" }), 60 * 30), JSON.stringify({ expiresAt: Date.now() + 1000 * 61, confirm: null, amount })),
            ...
            const row = createRow(
                new ButtonBuilder({
                    customId: `api/take-money/true/${transaction.id}`,
                    label: "Confirmar",
                    style: ButtonStyle.Success,
                }),
                new ButtonBuilder({
                    customId: `api/take-money/false/${transaction.id}`,
                    label: "Cancelar",
            ...
            if (channel.isSendable()) {
                ...
                return reply.status(StatusCodes.CREATED).send({
                    ...
                    success: true,
                    botBalance: application.data.money.toNumber(),
    ...
    createResponder({
        customId: "api/take-money/:confirm/:transactionId",
        ...
        async run(interaction, { confirm, transactionId }): Promise<any> {
            ...
            const transactionKey = `tx:${transaction.targetId}:${transaction.userId}`;
            ...
            const user = await prisma.user.findUnique({ where: { id: transaction.targetId } });
            if (!user || user.money.toNumber() < transaction.amount) {
                await redis.del(transactionKey);
